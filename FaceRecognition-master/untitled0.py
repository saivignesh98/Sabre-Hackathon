# -*- coding: utf-8 -*-
"""
Created on Wed Jan 15 23:47:11 2020
...
def faceDetection(test_img):
    gray_img=cv3.cvtColor(test_img,cv3.COLOR_BGR2GRAY)#convert color image to grayscale
    face_haar_cascade=cv3.CascadeClassifier('HaarCascade/haarcascade_frontalface_default.xml')#Load haar classifier
...
def labels_for_training_data(directory):
    ...
    for path,subdirnames,filenames in os.walk(directory):
        for filename in filenames:
            ...
            print("id:",id)
            test_img=cv3.imread(img_path)#loading each image one by one
            if test_img is None:
...
def train_classifier(faces,faceID):
    face_recognizer=cv3.face.LBPHFaceRecognizer_create()
...
def draw_rect(test_img,face):
    (x,y,w,h)=face
    cv3.rectangle(test_img,(x,y),(x+w,y+h),(255,0,0),thickness=5)

#Below function writes name of person for detected label
def put_text(test_img,text,x,y):
    cv3.putText(test_img,text,(x,y),cv3.FONT_HERSHEY_DUPLEX,2,(255,0,0),4)
